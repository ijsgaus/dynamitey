        ...
        public override bool TryInvoke(InvokeBinder binder, object[] args, out object result)
        {
            ...
            else if (args.Length == 2)
            {
                    ...
                    )
                {
                    ...
                    var tInterface = tEnumerableArg.GetType().GetTypeInfo().ImplementedInterfaces.FirstOrDefault(it=>it.Name =="IEnumerable`1");
                    if(tInterface !=null)
                    {
                        var tParamTypes = tInterface.GetTypeInfo().GenericTypeArguments;
                ...
                else if (Util.IsAnonymousType(args[1]))
                {
                    ...
                    foreach (var tProp in args[1].GetType().GetRuntimeProperties())
                    {
